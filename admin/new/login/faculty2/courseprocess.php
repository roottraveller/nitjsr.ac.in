...
$S_NO=$_POST['S_NO'];
$j=count($_POST['COURSE']);
for($i=0;$i<$j;$i++)
{
       $res1= mysql_query("UPDATE COURSE SET COURSE = '$COURSE[$i]' WHERE ID='$id' && S_NO = '$S_NO[$i]' " ) ;
	//echo $name;
    if(isset($_POST) and $_SERVER['REQUEST_METHOD'] == "POST")
        {
           $name = $_FILES['COURSEPDF']['name'][$i];
           // echo $name;
  ...
  // $actual_image_name = time().substr(str_replace(" ", "_", $txt), 5).".".$ext;
          ...
          $tmp = $_FILES['COURSEPDF']['tmp_name'][$i];
//echo $i;
...
//echo  $path.$actual_image_name ;
 ...
 $path2 = $path1.$id.'_'.$i.'_'.$name;
			...
			//echo $S_NO[$i];
                 ...
                 $res =  mysql_query("UPDATE COURSE SET COURSEPDF = '$path2'' WHERE ID='$id' && S_NO = '$S_NO[$i]' " ) ;

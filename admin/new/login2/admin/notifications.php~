...
function init()
{
	...
	{
		<?php 
	$num=mysql_num_rows(mysql_query("select * from ".$page." where status='1'  and 'time'> '2013-04-26' ORDER BY `time` DESC"));?>
	}
	else
	{<?php	
		$num2=mysql_num_rows(mysql_query("select * from ".$page." where status='0' and 'time'> '2013-04-26' ORDER BY `time` DESC")); ?>
	}
	total=<?php echo $num;?>;
